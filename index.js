		...
		try {
			...
			mess = {
				...
				only: {
					...
					ownerG: '⚠️ Este comando só pode ser usado pelo kant  ou bandida! 😂',
					ownerB: '❌ Este comando só pode ser usado pelo proprietário do bot! ❌',
					admin: '⚠️ Este comando só pode ser usado pelo o adm! 😝',
					Badmin: '❌ Este comando só pode ser usado quando eu virar adm amigo (a)❌'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5571982507697@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'info':
					...
					teks = `*𝑲𝑨𝑵𝑻 𝑳𝑺* : ${me.name}\n*48459081028* : @${me.jid.split('@')[0]}\n*Prefix* : ${prefix}\n*Contato de bloqueio total* : ${blocked.length}\n*O bot está ativo em* : ${kyun(uptime)}`
				...
				case 'sticker':
					...
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, 'Qual o código de idioma, 🤔?', text, {quoted: mek})
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do 𝑲𝑨𝑵𝑻 𝑳𝑺.*\n\n https://bit.ly/3iIlHmc'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
